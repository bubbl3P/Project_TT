...
$options_prices_net = array(
    'order_by' => 'id DESC',
    'where' => 'status=0 OR status=2'
);

$total_prices_net = get_total_prices_net('orders', $options_prices_net);

$options_prices_gross = array(
    'order_by' => 'id DESC',
    'where' => 'status=1'
);

$total_prices_gross = get_total_prices_gross('orders', $options_prices_gross);

$options_cod_net = array(
    'where' => 'type="offline" AND status=0 OR status=2'
);

$total_cod_net = get_total_cod_net('orders', $options_cod_net);

$options_cod_gross = array(
    'where' => 'status=1 AND type="offline"'
);

$total_cod_gross = get_total_cod_gross('orders', $options_cod_gross);

$options_vnpay_net = array (
    'where' => 'type="online" AND status=0 OR status=2'
);

$total_vnpay_net = get_total_vnpay_net('orders', $options_vnpay_net);

$options_vnpay_gross = array(
  'where' => 'status=1 AND type="online"'
);

$total_vnpay_gross = get_total_vnpay_gross('orders', $options_vnpay_gross);
